 ...
 * 1、无界阻塞队列，基于 BlockingQueue 实现，队列元素需要实现 Delayed 接口
 * 2、只有队列元素指定延迟时间到期才可以取到元素
 * 3、head 元素是已过期时间最长的元素
 * 4、如果没有元素过期，head 为null，调用 poll 方法返回 null
 * 5、如果调用 getDelay(TimeUnit.NANOSECONDS) 方法返回结果 <= 0，则该元素到期
 * 6、未到期的元素不能使用 take | poll 方法移除，他们只是一个普通元素。
 * 7、size 方法返回队列中到期和未到期元素个数总和
 * 8、DelayQueue 不可以存储 null 元素。
 *
