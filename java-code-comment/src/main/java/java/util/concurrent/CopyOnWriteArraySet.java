 ...
 *
 * CopyOnWriteArraySet 内部使用 CopyOnWriteArrayList 实现
 *
 ...
 *      array.
 *
 *      1、对于写操作（add、set、remove）等操作是线程安全的
 *      2、写操作会复制底层数组，所以是线程安全的
 *
 *  <li>Iterators do not support the mutative {@code remove} operation.
 *  1、在写操作（remove）过程中是不能够进行遍历操作的
 *
 *  <li>Traversal via iterators is fast and cannot encounter
 *      interference from other threads. Iterators rely on
 *      unchanging snapshots of the array at the time the iterators were
 *      constructed.
 *      1、通过迭代器进行的遍历操作速度很快，前提是没有遇到其他线程的干扰
 *      2、迭代器依赖于迭代器的数组快照来完成。
 * </ul>
 *
...
public class CopyOnWriteArraySet<E> extends AbstractSet<E>
    ...
    // CopyOnWriteArraySet 底层使用 CopyOnWriteArrayList实现
    private final CopyOnWriteArrayList<E> al;

    /**
     * Creates an empty set.
     * 创建空的 CopyOnWriteArraySet 对象
     */
     ...
     * collection.
     * 创建 CopyOnWriteArraySet 对象，将参数集合元素存储到set中
     *
