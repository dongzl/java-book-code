 ...
 * 1、使用 Array 数据结构实现的有界阻塞队列；
 * 2、入队、出队为 FIFO 原则；
 * 3、head 是入队时间最长元素，tail 是入队时间最短的元素；
 * 4、插入元素放到尾部，获取元素从头部获取
 *
 ...
 * 1、这是一个经典的有界缓冲队列实现，可以用于生产者/消费者模型中；
 * 2、一旦创建，无法修改容量大小；
 * 3、使用 put 方法向一个装满队列中添加元素，或者是从一个空队列中使用 take 方法获取元素，都会导致阻塞。
 *
